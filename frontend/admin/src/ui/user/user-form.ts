...
export default class UserForm extends Form {
	build(item: Entity, options: any) {
		this.addSection("User properties", FaIcon.s("user"))
			.addControl(controls.string("email", "e-mail").setRole("admin"))
			.addControl(controls.string("name", "name").setRole("admin"))
			.addControl(controls.password("password", "password").setRole("admin"))
