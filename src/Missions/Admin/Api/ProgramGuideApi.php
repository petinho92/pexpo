...
#[Goldify(Programguide::class, User::ROLE_WEBMASTER)]
class ProgramGuideApi extends Gold
{
        ...
        return new class($this, 50, true) extends ListApi {
            public function views(): array
            {
                return [
                    ...
                    new ListView("Active", fn() => Filter::where(Programguide::active(true))),
                    new ListView("Inactive", fn() => Filter::where(Programguide::active(false|null))),
                ];
            }
            ...
            {
                return [
                    new ListSorting("name", fn($asc) => $asc ? [[Programguide::name, "asc"]] : [[Programguide::name, "desc"]]),
                ];
            }

            public function export(Entity|Programguide $item): array
            {
            ...
            public function searchFilter(array $filter): Filter|null {
                return Filter::where(isset($filter['name']) && $filter['name'] ? Programguide::name()->instring($filter['name']) : null);
            }
            public function quickSearchFilter(string $search): Filter {
                return Filter::where(Programguide::name()->instring($search))
                    ->or(Programguide::id($search))
                    ;
            }
        };
    }
